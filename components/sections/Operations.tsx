...
export default function OperationsSection() {
  ...
  return (
    <section id="operations" className="w-full pt-32 pb-24 bg-[#03060a] min-h-screen font-mono text-white selection:bg-[#00d2ff] selection:text-black">
      
      {/* Header - Kept inside standard max-width */}
      <div className="max-w-7xl mx-auto px-6 mb-12 text-center md:text-left">
        <h1 className="text-3xl md:text-5xl font-black tracking-widest mb-4 uppercase text-[#00d2ff] drop-shadow-[0_0_15px_rgba(0,210,255,0.3)]">
      ...
      {/* 🎯 TOP STOPWATCH / WARNING SECTION - FULL SCREEN WIDTH */}
      <div className="w-full mb-16">
        {loading ? (
           <div className="py-10 animate-pulse text-[#00d2ff] font-bold tracking-widest text-center uppercase">Scanning_Network...</div>
        ) : allEvents.length === 0 ? (
          /* 🔴 RED WARNING */
          <div className="max-w-7xl mx-auto py-24 px-8 border border-[#ff5555]/30 bg-[#ff5555]/5 rounded-none text-center animate-fadeUp">
            <div className="mb-6 flex justify-center gap-3">
              <div className="w-2 h-2 bg-[#ff5555] rounded-full animate-pulse"></div>
              <div className="w-2 h-2 bg-[#ff5555] rounded-full animate-pulse delay-75"></div>
            </div>
            <h2 className="text-[#ff5555] font-black text-lg md:text-2xl tracking-[0.2em] md:tracking-[0.4em] uppercase mb-4 whitespace-nowrap">
              [ !! NO_UPCOMING_EVENTS !! ]
            </h2>
          ...
          /* 🔵 FULL WIDTH LONG RECTANGLE TIMER */
        /* 🔵 FULL WIDTH LONG RECTANGLE TIMER */
/* 🔵 SLIM LINEAR TIMER (H-12 STYLE) */
/* 🔵 SLIM CYAN RECTANGLE TIMER */
/* 🔵 SLIM CYAN RECTANGLE WITH BUTTON */
<div className="w-full mt-32 flex justify-center px-4">
  <div className="w-full max-w-6xl h-32 bg-[#05080c] border-y border-[#00d2ff]/10 flex items-center justify-between px-8 md:px-12 transition-all shadow-[0_0_50px_rgba(0,210,255,0.02)] relative overflow-hidden">
    ...
    {latestEvent.regLink && (
      <div className="flex items-center gap-8">
        ...
        <div className="hidden md:block h-12 w-[1px] bg-[#00d2ff]/10"></div>
        
        <Link 
          href={latestEvent.regLink} 
          target="_blank" 
          className="px-6 py-3 border border-[#00d2ff]/30 text-[#00d2ff] hover:bg-[#00d2ff] hover:text-black text-[10px] md:text-xs font-black transition-all duration-300 font-mono uppercase tracking-[0.2em] hover:shadow-[0_0_20px_rgba(0,210,255,0.4)] whitespace-nowrap"
        >
          // JOIN_OPERATION _
        </Link>
      </div>
      ...
      {/* Categories & Grid - Back inside standard max-width */}
      <div className="max-w-7xl mx-auto px-6">
        {allEvents.length > 0 && (
          <div className="flex flex-nowrap overflow-x-auto gap-3 mb-12 pb-4 scrollbar-hide">
            {categories.map((cat) => (
              <button key={cat} onClick={() => setFilter(cat)} className={`px-6 py-2 rounded-md text-xs font-bold whitespace-nowrap border transition-all uppercase tracking-widest ${filter === cat ? 'bg-[#00d2ff]/10 border-[#00d2ff] text-[#00d2ff] shadow-[0_0_15px_rgba(0,210,255,0.2)]' : 'bg-white/5 border-white/10 text-gray-500 hover:text-white'}`}>
                {cat}
              </button>
            ))}
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 animate-fadeUp">
            {filteredEvents.map((event) => (
              <div 
                ...
                className="w-full h-[580px] bg-[#0a0c10] border-2 border-[#00d2ff]/30 rounded-none overflow-hidden flex flex-col group grayscale hover:grayscale-0 transition-all duration-300 hover:shadow-[0_0_60px_rgba(0,210,255,0.2)] cursor-pointer relative"
              >
                <div className="bg-[#0f172a] px-3 py-2.5 flex justify-between items-center border-b border-[#00d2ff]/30 shrink-0">
                  ...
                  <div className="text-gray-400 text-[10px] font-mono tracking-widest uppercase">
                  ...
                  <div className="absolute bottom-0 left-0 w-full p-5 bg-gradient-to-t from-[#0a0c10] via-[#0a0c10]/80 to-transparent pt-20">
                    <div className="relative h-14 w-full flex items-center justify-center">
                      <div className="absolute inset-0 flex items-center justify-center transition-all duration-500 group-hover:opacity-0 group-hover:scale-95 group-hover:-translate-y-4">
                        <div className="px-6 py-3 border border-[#00d2ff]/40 bg-[#00d2ff]/5 text-[#00d2ff] font-mono font-bold text-xs uppercase tracking-[0.2em]">
      ...
      {/* DETAIL MODAL */}
      {selectedEvent && (
        <div className="fixed inset-0 z-[10000] flex items-center justify-center bg-black/90 p-4 md:p-8" onClick={() => setSelectedEvent(null)}>
          <div className="w-full max-w-[1100px] h-[90vh] bg-[#0B111A] border border-[#2a2e3f] rounded-none overflow-hidden flex flex-col relative" onClick={(e) => e.stopPropagation()}>
            <div className="bg-[#0f172a] px-4 py-3 flex justify-between items-center border-b border-[#2a2e3f] shrink-0 font-mono">
              ...
              <div className="text-gray-400 text-[11px] font-mono tracking-widest uppercase">{selectedEvent.title.toLowerCase().replace(/\s+/g, '_')}_node.bin</div>
            ...
            <div className="flex flex-col lg:flex-row flex-1 overflow-y-auto lg:overflow-hidden">
              <div className="lg:w-[45%] bg-black p-0 flex items-center justify-center border-b lg:border-b-0 lg:border-r border-[#2a2e3f] shrink-0">
                {selectedEvent.posterUrl ? (
                  <img src={selectedEvent.posterUrl} alt={selectedEvent.title} className="w-full h-full object-contain" />
                ) : (
                  <div className="text-gray-600 font-mono tracking-widest py-20 lg:py-0">[ NO_VISUAL_FEED ]</div>
                )}
              </div>

              <div className="lg:w-[55%] bg-[#0B111A] p-8 md:p-10 flex flex-col lg:overflow-y-auto custom-scrollbar">
                <div className="bg-white/5 text-[#00d2ff] text-[10px] md:text-xs font-bold px-4 py-1.5 rounded-none uppercase tracking-[0.2em] w-fit mb-6 border border-[#00d2ff]/30 font-mono">[ {selectedEvent.category.toUpperCase()} ]</div>
                <h2 className="text-white text-3xl md:text-4xl lg:text-5xl font-extrabold mb-4 leading-tight tracking-tight uppercase">{selectedEvent.title}</h2>
                <p className="text-[#4ade80] text-sm md:text-base font-semibold mb-8 font-mono tracking-wider">&gt; CHARUSAT_NODE_ACTIVE</p>

                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4 shrink-0">
                  <div className="bg-[#121824] p-5 border border-white/5 flex gap-4">
                    <div className="text-gray-500 text-[10px] uppercase font-bold tracking-widest mb-1">DATE</div>
                    <div className="text-white text-sm font-bold font-mono">{selectedEvent.date}</div>
                  </div>
                  <div className="bg-[#121824] p-5 border border-white/5 flex gap-4">
                    <div className="text-gray-500 text-[10px] uppercase font-bold tracking-widest mb-1">TIME</div>
                    <div className="text-white text-sm font-bold font-mono">{selectedEvent.startTime || "TBA"}</div>
                  </div>
                </div>

                <div className="bg-[#121824] p-5 border border-white/5 flex gap-4 mb-8 shrink-0">
                  <div className="text-gray-500 text-[10px] uppercase tracking-widest font-bold mb-1">LOCATION</div>
                  <div className="text-white text-sm font-bold leading-relaxed font-mono">{selectedEvent.venue || "ENCRYPTED_NODE"}</div>
                </div>

                <div className="mb-10">
                  <h3 className="text-[#4ade80] text-sm font-bold uppercase tracking-widest mb-4 font-mono">$&gt; MISSION_DESCRIPTION</h3>
                  <div className="text-gray-300 text-sm leading-relaxed whitespace-pre-wrap font-mono">{selectedEvent.description}</div>
                </div>

                <div className="mt-auto pt-8 shrink-0">
                  <a href={selectedEvent.regLink || "#"} target="_blank" rel="noopener noreferrer" className={`block w-full text-center py-4 font-bold font-mono uppercase tracking-[0.2em] transition-colors ${selectedEvent.regLink ? 'bg-[#16a34a] hover:bg-[#15803d] text-white' : 'bg-white/5 text-gray-500 cursor-not-allowed'}`}>
